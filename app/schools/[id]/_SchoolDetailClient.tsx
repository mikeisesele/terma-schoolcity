            ...
            {school.reviews > 0 ? (
              <>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', gap: 2, margin: '4px 0' }}>
                  {[1,2,3,4,5].map(s => <span key={s} style={{ fontSize: 13, color: s <= Math.round(school.rating) ? '#FCD34D' : 'rgba(255,255,255,.3)' }}>★</span>)}
                </div>
                <div style={{ fontSize: 11.5, color: 'rgba(255,255,255,.7)', fontWeight: 600 }}>{school.reviews} {school.reviews === 1 ? 'review' : 'reviews'}</div>
              </>
            ) : (
              <>
                <div style={{ fontSize: 11.5, color: 'rgba(255,255,255,.7)', fontWeight: 600 }}>No reviews yet</div>
              </>
            )}
          </button>
        </div>
          ...
          <div style={{ background: T.cardBg, borderRadius: 18, width: '100%', maxWidth: 600, maxHeight: '85vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 24px 64px rgba(0,0,0,.35)' }}>
            ...
            <div style={{ padding: '16px 24px', borderBottom: `1px solid ${T.line}`, display: 'flex', gap: 24, alignItems: 'center', flexShrink: 0 }}>
              <div style={{ textAlign: 'center' }}>
                <div style={{ fontSize: 48, fontWeight: 900, color: T.ink, lineHeight: 1 }}>{school.reviews > 0 ? school.rating.toFixed(1) : '—'}</div>
                <Stars rating={school.rating} />
                <div style={{ fontSize: 12, color: T.ink3, fontWeight: 600, marginTop: 3 }}>{school.reviews} {school.reviews === 1 ? 'review' : 'reviews'}</div>
              </div>
              <div style={{ flex: 1 }}>
