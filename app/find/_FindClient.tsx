...
function SNFindSchoolInner({ schoolCountLabel }: { schoolCountLabel: string }) {
  ...
  return (
    <div style={{ minHeight:'100vh', background:T.bg, fontFamily:T.font }} onClick={()=>setOpenF(null)}>
      ...
      <div style={{ background:'linear-gradient(135deg,'+T.accent+' 0%,'+T.accent+'ee 100%)', padding:'40px 48px 32px' }}>
        ...
        <p style={{ margin:'0 0 24px', fontSize:15, color:T.accentText+'80', textAlign:'center', fontFamily:T.font }}>Search across {schoolCountLabel} in Nigeria</p>
...
export function FindPageClient({ schoolCountLabel }: { schoolCountLabel: string }) {
  return (
    <Suspense>
      <SNFindSchoolInner schoolCountLabel={schoolCountLabel} />
    </Suspense>
  );
}
