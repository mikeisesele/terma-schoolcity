...
export function SCCard({ school, onSelect, isFav, onToggleFav, inCompare, onToggleCompare }: {
  ...
  const paidBadge = school.schoolcityTier === 'spotlight'
    ? { label: `★ ${scopeLabel}Spotlight`, bg: 'rgba(184,125,32,.92)' }
    : school.schoolcityTier === 'rated'
      ? { label: `✓ ${scopeLabel}Top-Rated`, bg: 'rgba(31,107,69,.9)' }
      : null;
  return (
...
export function SCCompareModal({ compareIds, allSchools, onClose, onRemove, onSelect }: {
  ...
  const rows: Array<{label:string;key:string;render:(s:School)=>React.ReactNode;compare?:(s:School)=>number;val?:(s:School)=>string}> = [
    ...
    { label:'Plan',           key:'plan',         render: s => <span style={{ fontSize:12, fontWeight:800, color: s.ktPlan==='Standard'?'#B87D20':ink3, background: s.ktPlan==='Standard'?'#FEF3C7':'#F3F4F6', borderRadius:4, padding:'2px 8px' }}>{s.ktPlan||'—'}</span>, val: s => s.ktPlan ?? '' },
  ];
