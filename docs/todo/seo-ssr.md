...
Spec: BUILD.md §0 (SSR profiles, ISR ≈60s) · `SYSTEM.md §9`/§3.12 · Phase 5 · `CLAUDE.md`. Design ref: `SchoolOS SchoolCity.html` (`<title>`, fonts).
...
- [ ] 🔴 **SSR school profile pages** — As a search engine, I can crawl fully-rendered school profiles so that schools rank in Google.
  ...
  - Spec: BUILD.md §0 · `SYSTEM.md §3.12` (`GET /api/schoolcity/schools/:id`)
  - Backend: `GET /api/schoolcity/schools/:id` → profile + published vacancies + open scholarships (verified+published only)
  - Gating/Auth: public
...
- [~] 🔴 **Per-page dynamic metadata** — As a visitor sharing a link, the title/description/OG reflect the specific school so that shares look good and rank well.
  ...
  - Backend: `GET /api/schoolcity/schools/:id`
  - Gating/Auth: public
  - Accept: `generateMetadata` per school — title `"{School} · {City} | SchoolOS SchoolCity"`, description from tagline/intro, canonical URL, OG image (banner/photo), Twitter card. Directory + Find pages have their own metadata.
...
- [~] 🔴 **sitemap.xml** — As a search engine, I can discover every public school so that the directory is fully indexed.
  ...
  - Backend: `GET /api/schoolcity/schools` (paginate all verified+published)
  - Gating/Auth: public
