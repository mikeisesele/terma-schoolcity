...
Spec: `SYSTEM.md §2.10` (`school_enquiries`, both phone+email required), §3.8 (`POST /api/enquiries`), §3.12 (`POST /api/schoolnet/enquire`), §2.14 (`source='schoolnet'`) · `DATA_SOURCES.md §7` · `UNWIRED_AUDIT.md` 🌐 School detail → Enquire Now · Phase 5. Design ref: `sn-masonry.jsx → SNDetail` (`enquireOpen`, `form {name, phone, email, message}`, `sent`).
...
- [ ] 🔴 **Enquire Now modal** — As a public visitor (parent), I can submit an enquiry to a school so that I can start a conversation about enrolling.
  ...
  - Spec: `SYSTEM.md §3.12` (`POST /api/schoolnet/enquire`), §2.10 · `UNWIRED_AUDIT.md` 🌐 ("Submit enquiry → toast only; needs `POST /api/public/schools/:id/enquire`")
  - Backend: `POST /api/schoolnet/enquire` `{ school_id, parent_name, phone, email, children_count, message }` → `school_enquiries` (source `schoolnet`); backend sends Termii SMS + SendGrid email to school (owned by kidtrack-backend)
  - Gating/Auth: public
  - Accept: modal with name, phone, email, message (and children_count if present); real POST replaces prototype toast-only; success + error states; `source=schoolnet` recorded server-side. (verify children_count field presence against SYSTEM.md §2.10 vs the prototype form)
...
- [ ] 🔴 **Required phone + email validation** — As the system, I require both contact channels so that the school can reply by SMS and email.
  ...
  - Backend: `POST /api/schoolnet/enquire` (server also validates)
  - Gating/Auth: public
...
- [ ] **Enquiry error handling** — As a parent, if my enquiry fails I can retry so that I'm not silently lost.
  ...
  - Backend: `POST /api/schoolnet/enquire` error responses
  - Gating/Auth: public
