...
Read moderated reviews sourced from the KidTrack Parent App. This repo **reads** only — no review authoring here. Respect anonymity.
...
> **Source-of-truth note:** Reviews are authored in the Parent App by a parent with an enrolled child (validated via `student_parents`) and pass a **moderation queue** before appearing publicly. The KidTrack team owns submission + moderation; School Net only renders approved reviews.
...
- [~] **No public review authoring** — As the system, I prevent review submission on School Net so that the moderated, enrolled-parent-only rule holds.
  ...
  - Gating/Auth: public
  - Accept: no review form/POST anywhere in this repo; any "rate this school" affordance routes to a "reviews come from KidTrack parents" explainer, not a submission form.
