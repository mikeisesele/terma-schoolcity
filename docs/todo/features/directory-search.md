...
Spec: `SYSTEM.md §9` (Home/Find pages), §3.12 (`GET /api/schoolcity/schools`) · `DATA_SOURCES.md §7` · `UNWIRED_AUDIT.md` 🌐 Home · Phase 5. Design ref: `sn-masonry.jsx → SNHome`, `SNFindSchool`, `SNCard`, `SHead`, `CAROUSEL`.
...
- [ ] 🔴 **Home hero carousel** — As a public visitor, I see featured schools in a rotating carousel so that I'm drawn into the directory.
  ...
  - Backend: `GET /api/schoolcity/schools` (featured = `schoolcity_verifications.badge_level` featured/premium) (verified+published only)
  - Gating/Auth: public
...
- [~] 🔴 **Highly-rated strip** — As a public visitor, I can browse top-rated schools so that I quickly find quality options.
  ...
  - Backend: `GET /api/schoolcity/schools` (sort by rating)
  - Gating/Auth: public
...
- [~] 🔴 **All Schools section** — As a public visitor, I can see the full directory so that I can browse everything available.
  ...
  - Backend: `GET /api/schoolcity/schools?page=&limit=`
  - Gating/Auth: public
...
- [~] 🔴 **Scholarships section** — As a public visitor, I can find schools offering scholarships so that affordability is visible up front.
  ...
  - Backend: `GET /api/schoolcity/schools` filtered to schools with open scholarships
  - Gating/Auth: public
...
- [~] 🔴 **Hiring Now section** — As a public visitor (job seeker), I can find schools with open vacancies so that I can apply.
  ...
  - Backend: `GET /api/schoolcity/schools` filtered to schools with published vacancies
  - Gating/Auth: public
...
- [~] 🔴 **Special Needs section** — As a public visitor (parent of a child with special needs), I can find inclusive schools so that I find a suitable match.
  ...
  - Backend: `GET /api/schoolcity/schools?special_needs=true`
  - Gating/Auth: public
...
- [~] 🔴 **SchoolOS marketing section on home** — As a parent who found a school, I learn SchoolOS benefits so that I'm primed to ask my school to adopt it.
  - Screens: `SNHome` SchoolOS marketing section
  - Spec: `SYSTEM.md §9` ("SchoolOS marketing section", 5 feature tiles, no Direct Messaging)
  - Backend: none (static content)
  - Gating/Auth: public
  - Accept: 5 tiles — Real-time bus tracking, Safety alerts, Results & report cards, Pay fees easily, Attendance & calendar; CTAs "Tell your school about SchoolOS" + "Learn more". No "List your school" CTA.
...
- [ ] 🔴 **Find a school — filters** — As a public visitor, I can filter schools so that I narrow to ones matching my needs.
  ...
  - Backend: `GET /api/schoolcity/schools?level=&type=&max_fee=&special_needs=&state=`
  - Gating/Auth: public
...
- [ ] 🔴 **Find a school — free-text search** — As a public visitor, I can search by name/keyword so that I jump straight to a known school.
  ...
  - Backend: `GET /api/schoolcity/schools?q=`
  - Gating/Auth: public
...
- [~] **Results grid empty + loading states** — As a visitor, I get clear feedback when results are loading or absent so that the page never looks broken.
  ...
  - Spec: Phase 5
  - Backend: `GET /api/schoolcity/schools`
  - Gating/Auth: public
