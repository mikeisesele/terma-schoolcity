# schoolnet-web — Build Plan (PM-as-files)

The complete task layer for the **KidTrack School Net** public directory. Each task is a checkable user story scoped for a Claude Code `build-session`. Implement top-down; verify against the spec before checking a box.

## Legend

- `- [ ]` open · `- [x]` done
- 🔴 = **MVP critical path** (BUILD.md §3 step 9: *School Net SSR + vacancy applications*). Ship these first.
...
2. **`01-auth.md`** — Google OAuth → `schoolnet_users`, sign-in gate, session, isolation from admin/parent JWT.
...
5. **`features/school-detail.md`** — profile tabs (About/Facilities/Vacancies/Scholarships/Map/Reviews) + KidTrack CTA.
...
- Design: `/Users/admin/Downloads/Unzip and extract continuation(2)/` → **live**: `Kidtrack School Net.html`, `sn-masonry.jsx`, `schoolnet-extras.jsx`. **Ignore** `SN Option*.html`, `schoolnet-options.dc.html`.
- `SYSTEM.md` — §9 (School Net arch), §2.10–§2.16 (schema), §3.8–§3.12 + §3.1 (API), §19 E/F/G.
- `DATA_SOURCES.md` §7. `UNWIRED_AUDIT.md` 🌐 School Net. `CONTINUATION.md` ENH-2. `VERDANT.md`.
...
1. No public self-listing — schools appear only via KidTrack onboarding.
2. `schoolnet_users` isolated from admin/parent identity.
